package com.br.minecraft.api.reflection;
...
public final class SmartScoreboardReflection {
    ...
    private SmartScoreboardReflection() {
    ...
    public static String nmsClassName(String post1_17package, String className) {
        if (NMS_REPACKAGED) {
            var classPackage = post1_17package == null ? NM_PACKAGE : NM_PACKAGE + '.' + post1_17package;
    ...
    public static Class<?> innerClass(Class<?> parentClass, Predicate<Class<?>> classPredicate) throws ClassNotFoundException {
        for (var innerClass : parentClass.getDeclaredClasses()) {
    ...
    public static PacketConstructor findPacketConstructor(Class<?> packetClass, Lookup lookup) throws Exception {
        ...
        if (theUnsafe == null) {
            synchronized (SmartScoreboardReflection.class) {
                if (theUnsafe == null) {
